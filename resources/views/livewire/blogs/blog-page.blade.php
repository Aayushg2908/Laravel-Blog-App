<div>
    ...
    <div class="mt-4 flex flex-col items-center justify-center">
        ...
        <form wire:submit="createComment" class="flex items-center gap-2">
            ...
            <x-text-input wire:model="comment" id="comment" name="comment" class="w-full"
                ...
                <div class="mt-4 flex items-center gap-4 justify-between bg-gray-100 p-2 rounded-lg">
                    ...
                    @if (auth()->user() && auth()->user()->id === $comment->user_id)
                        <div class="flex items-center gap-2 justify-center">
                            <x-secondary-button x-on:click.prevent="$dispatch('open-modal', 'edit-comment')"
                                wire:click="confirmEdit({{ $comment->id }})">Edit</x-secondary-button>
                            <x-danger-button x-on:click.prevent="$dispatch('open-modal', 'delete-comment')"
                                wire:click="confirmDelete({{ $comment->id }})">Delete</x-danger-button>
                        </div>
                    @endif
                </div>
            @endforeach
            ...
            <form wire:submit.prevent="updateBlog" class="p-6">
                ...
                <div class="mt-6 space-y-6">
                    <div>
                        ...
                        <x-text-input wire:model="editingTitle" id="editingTitle" name="editingTitle"
                            class="mt-1 block w-full" type="text" required />
                        <x-input-error :messages="$errors->get('editingTitle')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="editingContent" value="{{ __('Content') }}" />
                        <x-text-input wire:model="editingContent" id="editingContent" name="editingContent"
                            class="mt-1 block w-full" type="text" required />
        ...
        <x-modal name="edit-comment" focusable>
            <form wire:submit.prevent="updateComment" class="p-6">
                ...
                <div class="mt-6 space-y-6">
                    ...
                    <x-text-input wire:model="editingCommentContent" id="editingCommentContent"
                        name="editingCommentContent" class="mt-1 block w-full" type="text" required />
